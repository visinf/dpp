#ifndef VISINF_COMMON_H
#define VISINF_COMMON_H

// CUDA: grid stride looping
#define CUDA_KERNEL_LOOP(i, n) \
  for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < (n); i += blockDim.x * gridDim.x)

#define VISINF_assertSameGPU(...) THAssertMsg(THCTensor_(checkGPU)(__VA_ARGS__), \
...
#define VISINF_resizeAs_indices(STATE, I1, I2)              \
...
#define VISINF_check_shape(STATE, I1, I2)                 \
...
#define VISINF_check_shape_indices(STATE, I1, I2)              \
  THLongStorage *size2 = THCTensor_(newSizeOf)(STATE, I2);     \
  if (!THCIndexTensor_(isSize)(STATE, I1, size2))              \
  { \
...
#define VISINF_check_nElement(STATE, I1, I2)                \
  if (I1 != NULL && I2 != NULL ) {                          \
...
#define VISINF_check_dim_size(STATE, T, DIM, DIM_SIZE, SIZE) \
...
#define VISINF_check_dim_size_indices(STATE, T, DIM, DIM_SIZE, SIZE)  \
...
#define VISINF_argCheck(STATE, COND, ARG, T, FORMAT) \
  if (!(COND)) { \
    THCDescBuff s1 = THCTensor_(sizeDesc)(state, T); \
