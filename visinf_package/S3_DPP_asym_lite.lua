...
local S3_DPP_asym_lite, Parent = torch.class('visinf.S3_DPP_asym_lite','nn.Sequential')


function S3_DPP_asym_lite:__init(nPlane)
   Parent.__init(self)

   local model=nn.Sequential()
   local block=nn.ConcatTable()
   local DPP1=visinf.DPP_asym_lite(nPlane)
   local DPP2=visinf.DPP_asym_lite(nPlane)
   local DPP3=visinf.DPP_asym_lite(nPlane)
   local DPP4=visinf.DPP_asym_lite(nPlane)
