...
local S3_DPP_asym_full, Parent = torch.class('visinf.S3_DPP_asym_full','nn.Sequential')


function S3_DPP_asym_full:__init(nPlane)
   Parent.__init(self)

   local model=nn.Sequential()
   local block=nn.ConcatTable()
   local DPP1=visinf.DPP_asym_full(nPlane)
   local DPP2=visinf.DPP_asym_full(nPlane)
   local DPP3=visinf.DPP_asym_full(nPlane)
   local DPP4=visinf.DPP_asym_full(nPlane)
   ...
   DPP2.modules[1].modules[12]:share(DPP1.modules[1].modules[12],'weight','gradWeight')
   DPP3.modules[1].modules[12]:share(DPP1.modules[1].modules[12],'weight','gradWeight')
   DPP4.modules[1].modules[12]:share(DPP1.modules[1].modules[12],'weight','gradWeight')
